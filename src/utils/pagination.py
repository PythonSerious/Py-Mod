...
class AutoEmbedPaginator(object):
    ...
    async def run(self, embeds, send_to=0):
        if send_to == 0:
            send_to = self.interaction.response.send_message
        elif send_to == 1:
            send_to = self.interaction.channel.send
        elif send_to == 2:
            send_to = self.interaction.user.send
        else:
            logger.log("Invalid send_to value", logger.logtypes.error)
            return "Invalid send_to value."
        wait_for = self.interaction.user
        if not self.embeds:
            self.embeds = embeds
        if self.auto_footer:
            self.embeds[0].set_footer(text=f'({self.current_page + 1}/{len(self.embeds)})')
        msg = await send_to(embed=self.embeds[0])
        if send_to == 0:
...
class CustomEmbedPaginator(object):
    ...
    async def run(self, embeds, send_to=0):
        ...
        if send_to == 0:
            ...
            logger.log("Invalid send_to value. - E02", logger.logtypes.error)
            return "Invalid send_to value."
        wait_for = self.interaction.user
        if self.auto_footer:
            self.embeds[0].set_footer(text=f'({self.current_page + 1}/{len(self.embeds)})')
